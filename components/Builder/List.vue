...
const fields = ref(["name", "email", "phone number"]);
const emit = defineEmits(["emitData"]);
const props = defineProps({
  builder: { type: Array, required: true },
});
const editBuilder = ref({});
const edit = ref(false);
const emitData = (builder: any, index: any, value: any) => {
  if (value == "edit") {
    editBuilder.value = JSON.parse(JSON.stringify(builder));
    edit.value = true;
    return;
  }
  emit("emitData", { builder: builder, value: value, index: index });
};
</script>
