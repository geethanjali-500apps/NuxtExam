...
const fields = ref(["name", "status", "listing_type_name", "approve_status"]);
...
const emitData = (project: any, index: any, value: any) => {
  if (value == "edit") {
    editProject.value = JSON.parse(JSON.stringify(project));
    edit.value = true;
    return;
  }
  emit("emitData", { project: project, value: value, index: index });
};
</script>
